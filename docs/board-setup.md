...
- **`apt upgrade` can touch the kernel, and PYNQ is bound to it.** The device-tree overlay interface `insert_dtbo.py` writes to (`/sys/kernel/config/device-tree/overlays/`) is a Xilinx downstream patch, absent from a generic Ubuntu kernel, and `zocl` ships with the Kria kernel package. After upgrading, check `uname -r` still names a `-xilinx-zynqmp` flavour before assuming PYNQ is broken. `sudo apt-mark hold linux-image-xilinx-zynqmp linux-headers-xilinx-zynqmp` avoids the question entirely; userspace upgrades stay fine.
