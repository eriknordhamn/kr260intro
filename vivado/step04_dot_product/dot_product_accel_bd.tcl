...
################################################################
# Step 04: dot-product accelerator block design.
#
# !!! PROVISIONAL - NOT VALIDATED, NOT GUI-EXPORTED !!!
#
# This is step 03's exported dma_loopback_bd.tcl with the kernel spliced in
# by hand. Vivado has never parsed it. It exists as a reference for what the
# finished design should contain, and as a fallback; the real version is the
# File -> Export Block Design as TCL output from the GUI session described in
# TODO_NEXT.md, which should overwrite this file wholesale (do not merge the
# export into these edits by hand).
#
# Unverified specifically: whether the kernel's interface pins resolve under
# these lowercase names in IP Integrator, and whether validate_bd_design
# accepts the result.
#
# Intended changes versus step 03:
#   - dot_product_0 (kr260intro.local:user:dot_product:1.0) instantiated
#   - the DMA's MM2S->S2MM loopback wire split so the stream runs
#       axi_dma_0/M_AXIS_MM2S -> dot_product_0/s_axis
#       dot_product_0/m_axis  -> axi_dma_0/S_AXIS_S2MM
#   - dot_product_0's aclk/aresetn joined to the existing clock and reset
#     nets (pl_clk0 / rst_ps8_0_99M peripheral_aresetn)
#
# Address map is unchanged: the kernel has no memory-mapped interface, so
# only the DMA's S_AXI_LITE needs an address.
#
...
\
  ...
  # Create instance: dot_product_0, and set properties
  set dot_product_0 [ create_bd_cell -type ip -vlnv kr260intro.local:user:dot_product:1.0 dot_product_0 ]


  # Create interface connections
  # The step 03 loopback wire, split so the stream passes through the kernel.
  connect_bd_intf_net -intf_net axi_dma_0_M_AXIS_MM2S [get_bd_intf_pins axi_dma_0/M_AXIS_MM2S] [get_bd_intf_pins dot_product_0/s_axis]
  connect_bd_intf_net -intf_net dot_product_0_m_axis [get_bd_intf_pins dot_product_0/m_axis] [get_bd_intf_pins axi_dma_0/S_AXIS_S2MM]
  ...
  [get_bd_pins dot_product_0/aresetn]
  ...
  [get_bd_pins dot_product_0/aclk] \
